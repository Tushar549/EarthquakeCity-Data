...
public class EarthquakeCityMap extends PApplet {
	...
	private void sortAndPrint(int numToPrint) {
		...
		int count=0;
		if(numToPrint>eqm.size())
			numToPrint=eqm.size();
		for(EarthquakeMarker m:eqm) {
			if(count==numToPrint) {
				break;
			}
	...
	private void addKey() {
		 ...
		 * CityMarker.TRI_SIZE, tri_ybase + CityMarker.TRI_SIZE, tri_xbase +
		 * CityMarker.TRI_SIZE, tri_ybase + CityMarker.TRI_SIZE);
		 */
		 PImage pi=loadImage("https://media.contentapi.ea.com/content/dam/gin/images/2017/01/simcity-3000-unlimited-keyart.jpg.adapt.crop1x1.767p.jpg", "jpg");
		 pi.resize(14, 14);
		 image(pi,tri_xbase - CityMarker.TRI_SIZE, tri_ybase - CityMarker.TRI_SIZE);
		fill(0, 0, 0);
		textAlign(LEFT, CENTER);
